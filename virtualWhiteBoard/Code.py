...
# directory = r"C:\Users\ayush\virtualWhiteBoard-master\savedFile"

curr = os.getcwd()  # return current working directory
child_dir = "savedFiles"
directory = os.path.join(curr, child_dir)
nw = datetime.now()
curr_time = nw.strftime("%H_%M_%S")
...
# theme = 1 => black background and theme = 0 => white background
theme = 0
...
"""
    Since you are using np.zeros, you are explicitly telling it to fill the complete image with zeros. Adding (0,0,0) 
    at every position in the image results in a black image. Setting it all to (255,255,255) would be white, 
    equalling np.ones() function. If you want color then you should assign values to the image positions. 
"""
...
def whiteBoard(color=255, ):
    ...
    # for black circle and white text
    # theme = 0, white background
    if (color == 255):
        # cv2.circle(image, center_coordinate, radius_in_pixels, color_of_the_circle_in_B_R_G,
        #               thickness_of_the_circle_-1_for_filled_circle)
        # it will create black circles
        ...
        cv2.putText(virtualWhiteBoard, "THEME", (20 + 80, x_width - 30), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 255, 255),
                    1, cv2.LINE_AA)
        cv2.putText(virtualWhiteBoard, "CLEAR", (100 + 80, x_width - 30), cv2.FONT_HERSHEY_SIMPLEX, 0.4,
                    (255, 255, 255), 1, cv2.LINE_AA)

    # for white circle and black text
    # theme = 1, black background
        ...
        cv2.putText(virtualWhiteBoard, "THEME", (20 + 80, x_width - 30), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 0, 0), 1,
                    cv2.LINE_AA)
        cv2.putText(virtualWhiteBoard, "CLEAR", (100 + 80, x_width - 30), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 0, 0), 1,
                    cv2.LINE_AA)
    ...
    cv2.putText(virtualWhiteBoard, "RED", (180 + 80, x_width - 30), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 0, 0), 1,
                cv2.LINE_AA)
    cv2.putText(virtualWhiteBoard, "YELLOW", (260 + 80, x_width - 30), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 0, 0), 1,
                cv2.LINE_AA)
    cv2.putText(virtualWhiteBoard, "GREEN", (340 + 80, x_width - 30), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 0, 0), 1,
                cv2.LINE_AA)
    cv2.putText(virtualWhiteBoard, "BLUE", (420 + 80, x_width - 30), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 0, 0), 1,
                cv2.LINE_AA)
    cv2.putText(virtualWhiteBoard, "SAVE", (500 + 80, x_width - 30), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 0, 0), 1,
                cv2.LINE_AA)
...
def track():
    ...
    cv2.putText(Tracking, "THEME", (20 + 80, x_width - 30), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 255, 255), 1,
                cv2.LINE_AA)
    cv2.putText(Tracking, "CLEAR", (100 + 80, x_width - 30), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 255, 255), 1,
                cv2.LINE_AA)
    ...
    cv2.putText(Tracking, "BLUE", (420 + 80, x_width - 30), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (150, 150, 150), 1,
                cv2.LINE_AA)
...
while True:
    ...
    # Check to see if we have reached the end of the video or to check weather the reading was successful or
    if not grabbed:
        break
